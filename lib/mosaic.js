...
(function(){
  ...
  //   opts : { eagerFirst (def 4), durBase (def 150), assetBase (def 'assets/') }
      ...
      for (var rep = 0; rep < 2; rep++){
        for (var k = 0; k < reps; k++){
          for (var i = 0; i < cells.length; i++){
            ...
            var img = document.createElement('img');
            img.src = assetBase + code + '.webp'; img.alt = ''; img.decoding = 'async';
            if (rep === 0 && k === 0 && made < eagerFirst){ img.loading='eager'; img.fetchPriority='high'; }
            else { img.loading='lazy'; }
            card.appendChild(img);
            lane.appendChild(card);
            if (rep === 0 && k === 0) made++;
