      ...
      for (var rep = 0; rep < 2; rep++){
        for (var k = 0; k < reps; k++){
          for (var i = 0; i < cells.length; i++){
            ...
            // Only the genuine LCP tile(s) load eagerly at high priority; every other tile is
            // lazy + low so nothing competes with the hero for the first bytes off the wire.
            if (heroSet[code] && rep === 0 && k === 0){ img.loading='eager'; img.fetchPriority='high'; }
